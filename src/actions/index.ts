...
>> = () => {
  return (dispatch) => {
    dispatch(fetchModelAssetClassStart());
    getData('/api/v1/modelAssetClass/')
      .then((response) => dispatch(fetchModelAssetClassSuccess(response)))
      .catch((error) => dispatch(fetchModelAssetClassError(error)));
  };
};
...
export const fetchModelAssetClassStart: ActionCreator<Action> = () => ({
  type: 'FETCH_MODEL_ASSET_CLASS_START',
});

export const fetchModelAssetClassSuccess: ActionCreator<Action> = (
  payload,
) => ({
  type: 'FETCH_MODEL_ASSET_CLASS_SUCCESS',
  payload,
});

export const fetchModelAssetClassError: ActionCreator<Action> = (payload) => ({
  type: 'FETCH_MODEL_ASSET_CLASS_ERROR',
  payload,
});
